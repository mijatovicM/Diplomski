    ...
    if ($result->rowCount() > 0) {
      ?>
        ...
        if(isset($_POST['newpassword-submit'])) {
          ...
          echo '<script>location.replace("'.$url.'&error=passwordcheck&csrfToken=".generateCsrfToken());</script>';

                exit();
            } elseif (strlen($_POST['pwd']) < 6) {
                echo '<script>location.replace("'.$url.'&error=passwordlength&csrfToken=".generateCsrfToken());</script>';
                exit();
            }
